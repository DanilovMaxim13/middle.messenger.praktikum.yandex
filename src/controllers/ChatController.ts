...
import { sortByLastMessage } from '../services/helpers';
...
class ChatController {
    ...
    public async getUserChats(): Promise<Promise<null | undefined> | void> {
        try {
            const res = (await chatApi.getChats()) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
            }
    ...
    public async createChat(
        ...
        try {
            const res = (await chatApi.create(title)) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
            }
    ...
    public async getToken(chatId: number) {
        try {
            const res = (await chatApi.getToken(chatId)) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
            }
    ...
    public async changeChatAvatar(formData: FormData) {
        try {
            const res = (await chatApi.setChatAvatar(
                formData
            )) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
    ...
    public async addRemoveUsers(
        chatId: number,
        users: number[],
        add: boolean
    ): Promise<Promise<null | undefined> | void | string> {
        try {
            const res = (await chatApi.addRemoveUsers(
                chatId,
                users,
                add
            )) as XMLHttpRequest;
    ...
    public async searchUser(login: string) {
        try {
            const res = (await chatApi.searchUser(login)) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
            }
            return JSON.parse(res.responseText);
            ...
            try {
                ...
                if (data.type === 'message' || Array.isArray(data)) {
                    if (Array.isArray(data)) {
                        ...
                        const storeMessages =
                            JSON.parse(store.getState().messages as string) ||
                            [];
    ...
    public async deleteChat(
        chatId: number
    ): Promise<Promise<null | undefined | { result: { id: number } }> | void> {
        try {
            const res = (await chatApi.delete(chatId)) as XMLHttpRequest;
            if (res.status !== 200) {
                return null;
            }
            return JSON.parse(res.responseText);
